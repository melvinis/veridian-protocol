...
export default function FolderPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const resolvedFolder = getFolderById(folderId);
      if (!resolvedFolder) { router.push("/data-room"); return; }
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#05050f", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ color: "#4a5a75", fontSize: "0.85rem" }}>Loading folder…</p>
        </div>
    ...
    <div style={{ minHeight: "100vh", background: "#05050f", position: "relative" }}>
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "3rem 2rem 5rem" }}>
        ...
        <div style={{ marginBottom: "2.5rem" }}>
          <Link href="/data-room" style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", color: "#5a6a85", fontSize: "0.8rem", textDecoration: "none", marginBottom: "1.25rem" }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.75rem" }}>
            ...
            <div>
              <h1 style={{ fontSize: "clamp(1.4rem, 3vw, 1.9rem)", fontWeight: 800, color: "#f0f2f8", letterSpacing: "-0.02em", lineHeight: 1.15 }}>{folder.name}</h1>
              <p style={{ color: "#8b9bb8", fontSize: "0.875rem", marginTop: "0.25rem" }}>{folder.description}</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card" style={{ borderRadius: "16px", overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: "1rem", padding: "0.875rem 1.5rem", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
          ...
          {folder.documents.map((doc: DataRoomDocument, i: number) => {
            ...
            return (
              <div
                key={doc.id}
                style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: "1rem", padding: "1.25rem 1.5rem", borderBottom: i < folder.documents.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none", alignItems: "center", transition: "background 0.15s" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.02)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
              >
                <div style={{ display: "flex", alignItems: "flex-start", gap: "0.875rem" }}>
                  <div style={{ width: "36px", height: "36px", borderRadius: "8px", flexShrink: 0, background: `${typeColor}12`, border: `1px solid ${typeColor}25`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.62rem", fontWeight: 800, color: typeColor, letterSpacing: "0.04em" }}>
                    {doc.type}
                  </div>
                  <div>
                    <div style={{ color: "#f0f2f8", fontWeight: 600, fontSize: "0.9rem", marginBottom: "0.2rem" }}>{doc.name}</div>
                    <div style={{ color: "#5a6a85", fontSize: "0.77rem", lineHeight: 1.5 }}>
                      {doc.description}
                      {doc.pages && <span style={{ color: "#3a4a60", marginLeft: "0.5rem" }}>· {doc.pages} {doc.pages === 1 ? "page" : "pages"}</span>}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ textAlign: "right", minWidth: "100px" }}>
                  {doc.available ? (
                    ...
                    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", color: "#3a4a60", fontSize: "0.75rem", fontWeight: 600, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "6px", padding: "0.35rem 0.75rem" }}>
                      Coming soon
                    </span>
                  )}
                </div>
        ...
        {/* NDA notice */}
        <div style={{ marginTop: "1.5rem", display: "flex", gap: "0.75rem", alignItems: "flex-start", background: "rgba(212,168,67,0.04)", border: "1px solid rgba(212,168,67,0.1)", borderRadius: "10px", padding: "1rem 1.25rem" }}>
