  ...
  return (
    <div style={{ minHeight: "100vh", background: "#05050f", position: "relative" }}>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "3rem 2rem 5rem" }}>
        ...
        {locked.length > 0 && (
          <div>
            <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#3a4a60", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: "1.5rem" }}>Restricted Folders</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1rem" }}>
              {locked.map((folder: DataRoomFolder) => (
                <div key={folder.id} style={{ borderRadius: "14px", padding: "1.5rem", background: "rgba(5,5,15,0.6)", border: "1px solid rgba(255,255,255,0.04)", opacity: 0.5 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
                    <span style={{ fontSize: "2rem", filter: "grayscale(1)", opacity: 0.4 }}>{folder.icon}</span>
                    <span style={{ fontSize: "0.7rem", color: "#3a4a60", background: "rgba(255,255,255,0.03)", padding: "0.2rem 0.55rem", borderRadius: "999px", border: "1px solid rgba(255,255,255,0.06)", fontWeight: 600 }}>🔒 Restricted</span>
                  </div>
                  <h3 style={{ color: "#4a5a70", fontWeight: 700, fontSize: "0.95rem", marginBottom: "0.5rem" }}>{folder.name}</h3>
                  <p style={{ color: "#2a3a50", fontSize: "0.78rem", lineHeight: 1.6 }}>{folder.description}</p>
                </div>
              ))}
            </div>
            <p style={{ color: "#2a3a50", fontSize: "0.75rem", marginTop: "1.25rem", textAlign: "center" }}>
              To request elevated access, contact{" "}
              <a href="mailto:ir@veridianprotocol.com" style={{ color: "#4a5a70", textDecoration: "none" }}>ir@veridianprotocol.com</a>
            </p>
          </div>
        )}
      </div>
